...
$vSQL_1 = "SELECT a.*, b.name as cate_name FROM product as a
	INNER JOIN category as b ON b.id = a.category_id $vSQL_Search";
...
$vSQL_Sel = "SELECT a.*, b.name as cate_name FROM product as a
	INNER JOIN category as b ON b.id = a.category_id 
    $vSQL_Search
	ORDER BY a.name ASC
...
<?php
switch ($vAct) {
    case 'add' : include_once 'product_add.php';
        break;
    case 'edit' : include_once 'product_edit.php';
        break;
    default :
        ?>
        ...
        <div class="row">
            <div class="col-sm-6">
                <a href="?mod=<?= $vModule; ?>&act=add" class = "btn btn-info" title="Thêm mới loại sản phẩm" ><i class="fa fa-plus-square bigger-130"></i> Add product</a>
                            ...
                            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="hidden-480"><a href="#">Category</a></th>
                                        <th class="hidden-480"><a href="#">SKU</a></th>
                                        <th class="hidden-480"><a href="#">Description</a></th>
                                        <th class="hidden-480"><a href="#">Price</a></th>
                                    ...
                                    if (mysql_num_rows($vResult_Sel) > 0) {
                                        while ($category = mysql_fetch_array($vResult_Sel)) {
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="#"><?= $category['sku'] ?></a>
                                                </td>
                                                <td>
                                                    <a href="#"><?= $category['description'] ?></a>
                                                </td>
                                                <td>
                                                    <a href="#"><?= $category['price'] ?></a>
                                                </td>
                                        ...
                                        <?php } ?>
                                    <?php } else { ?>
                                        <tr><td colspan="8"><i>Không tồn tại dữ liệu!!</i></td></tr>
                                    <?php } ?>
                                </tbody>
                            </table>
                        </div>
