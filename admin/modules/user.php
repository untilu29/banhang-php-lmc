...
if (isset($_POST["search_submit"])) {
    ...
    $vSQL_Search = "WHERE (a.name LIKE '%$searchtext%')
	    		    or (a.username LIKE '%$searchtext%')
                                    or (a.email LIKE '%$searchtext%')";
}

//3. xác định tổng số bảng ghi
$vSQL_1 = "SELECT a.*, b.name as role_name FROM users as a
	INNER JOIN role as b ON b.id = a.role_id $vSQL_Search";
...
//Trường hợp lấp danh sách user (theo phân trang)
$vSQL_Sel = "SELECT a.*, b.name as role_name FROM users as a
	INNER JOIN role as b ON b.id = a.role_id 
    $vSQL_Search
	ORDER BY a.name ASC
...
<?php
switch ($vAct) {
    case 'add' : include_once 'user_add.php';
        break;
    case 'edit' : include_once 'user_edit.php';
        break;
    default :
        ?>
        ...
        <div class="row">
            <div class="col-sm-6">
                <a href="?mod=<?= $vModule; ?>&act=add" class = "btn btn-info" title="Thêm mới tài khoản" ><i class="fa fa-plus-square bigger-130"></i> Add user</a>
                            ...
                            <table id="dynamic-table" class="table table-striped table-bordered table-hover">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="hidden-480"><a href="#">Username</a></th>
                                        <th class="hidden-480"><a href="#">Name</a></th>
                                        <th class="hidden-480"><a href="#">Email</a></th>
                                        <th class="hidden-480"><a href="#">Role Name</a></th>
                                    ...
                                    if (mysql_num_rows($vResult_Sel) > 0) {
                                        while ($row = mysql_fetch_array($vResult_Sel)) {
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="#"><?= $row['role_name'] ?></a>
                                                </td>
                                                <td class="hidden-480 center">
                                                    <a class="hide_show" href="javascript:;" showid="<?php echo $row['id']; ?>" showac="<?php echo $row['actived']; ?>">
                                                        <?php if ($row['actived']) { ?>
                                                ...
                                                <td class="center">
                                                    <div class="hidden-sm hidden-xs action-buttons">
                                                        <a class="green" <a href="?mod=<?php echo $vModule; ?>&act=edit&id=<?php echo $row["id"]; ?>" title="Edit">
                                                                <i class="ace-icon fa fa-pencil bigger-130"></i>
                                                            </a>

                                                            <a data-toggle="modal" data-target="#deleteModal" onclick="delete_item(<?=$row['id']?>)" class="red" href="javascript:;" title="Delete">
                                        ...
                                        <?php } ?>
                                    <?php } else { ?>
                                        <tr><td colspan="7"><i>Không tồn tại dữ liệu!!</i></td></tr>
                                    <?php } ?>
                                </tbody>
                            </table>
                        </div>
...
<div id="deleteModal" class="modal fade">
    <div class="modal-dialog">
        ...
        <form action="controllers/user.php" method="POST" class="form-horizontal">
